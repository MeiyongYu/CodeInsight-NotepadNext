...
QVector<ProjectManager::ProjectSymbol> ProjectManager::findCallers(const QString &name, QString *errorMessage) const
{
    ...
    if (!hasProject()) {
        if (errorMessage)
            *errorMessage = tr("Finding callers needs an open project: the call sites come from its cscope database.");
        return callers;
    }
    ...
    // must never touch the file); -L: plain text on stdout, no TUI; -3:
    // "functions calling this function". The lookup is an exact symbol match,
    // case sensitive, and a name inside a string literal is not a call, so no
    ...
    // One line per call site:  <file> <caller> <line> <source text>
    ...
    // name and a number, which is exactly the file/caller/line split. Anything
    // after that number is the source text and is kept as is.
    ...
    std::sort(callers.begin(), callers.end(), [](const ProjectSymbol &a, const ProjectSymbol &b) {
