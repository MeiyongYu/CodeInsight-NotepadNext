...
public:
    ...
    // The flip side of findCallers (cscope query 2, "functions called by this
    // function"): every function the given one calls, one entry per call site,
    // sorted the same way. name is the callee field, file/line point at the
    // line inside the given function that does the call.
    QVector<ProjectSymbol> findCallees(const QString &name, QString *errorMessage = nullptr) const;
...
private:
    ...
    // The body findCallers() and findCallees() share: checks, the cscope -L
    // launch and the output parsing. query is the cscope query number
    // (3 = callers of the symbol, 2 = functions it calls); what names the
    // relation in the error texts ("callers", "callees").
    QVector<ProjectSymbol> queryRelations(int query, const QString &what, const QString &name, QString *errorMessage) const;
