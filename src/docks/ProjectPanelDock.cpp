      ...
      manager(projectManager)
{
    ...
    fileFilter->setPlaceholderText(tr("File Name or Path (space = AND)"));
    ...
    // Every column boundary is draggable: a Stretch section (the previous
    // setting of the Directory column) refuses to be resized by the user.
    // The last column keeps stretching to absorb the leftover width instead.
    fileTree->header()->setStretchLastSection(true);
    fileTree->header()->setSectionResizeMode(QHeaderView::Interactive);
    fileTree->header()->resizeSection(0, 180);
    fileTree->header()->resizeSection(1, 280);
    fileTree->header()->resizeSection(2, 70);
...
{
    fileTree->clear();
    // Combined search: the box may hold several space separated keywords and a
    // file is listed only when every one of them occurs in its full path (case
    // insensitive, the order does not matter). "tcp ipv4" therefore finds both
    // tcp_ipv4.c and net/ipv4/tcp.c - the directory may supply a keyword.
    const QStringList needles = fileFilter->text().simplified().split(QLatin1Char(' '), Qt::SkipEmptyParts);
    const QStringList files = manager->projectFiles();
    const QString sourceRoot = manager->sourceRoot();

    for (const QString &f : files) {
        const QFileInfo info(f);
        if (!needles.isEmpty()) {
            // Accept a keyword in either separator style: the list shows
            // native paths, the project file list stores forward slashes.
            const QString path = info.absoluteFilePath();
            const QString nativePath = QDir::toNativeSeparators(path);
            const bool matched = std::all_of(needles.cbegin(), needles.cend(), [&path, &nativePath](const QString &needle) {
                return path.contains(needle, Qt::CaseInsensitive) || nativePath.contains(needle, Qt::CaseInsensitive);
            });
            if (!matched)
                continue;
        }
        ...
        // The Directory column shows the path relative to the project source
        // root; a file that lives outside of the root keeps its full path.
        if (sourceRoot.isEmpty()) {
            item->setText(1, QDir::toNativeSeparators(info.absolutePath()));
        }
        else {
            const QString rel = QDir(sourceRoot).relativeFilePath(info.absoluteFilePath());
            item->setText(1, rel.startsWith(QLatin1String("..")) || QDir::isAbsolutePath(rel)
                ? QDir::toNativeSeparators(info.absolutePath())
                : QDir::toNativeSeparators(rel));
        }
